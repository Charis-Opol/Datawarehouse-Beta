...
async def get_query_data(query_id: str):
    ...
    try:
        ...
        # This endpoint is deprecated, but applying the fix for completeness.
        # The main server is now ecowarehouse.api.server
        df_cleaned = df.replace([np.inf, -np.inf, np.nan], None)
